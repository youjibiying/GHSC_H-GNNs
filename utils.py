...
class Logger:
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            print(f"   Final Test: {result['test_mean']:.2f} \p {result['test_std']:.2f}")
        return result
...
import  pickle
def get_split_from_path(file):
    print("loading exist split from", file)
    with open(file, 'rb') as H:
        Splits = pickle.load(H)
        train, test, val= Splits['train'], Splits['test'],Splits['test']

        split_idx = {
            'train': torch.tensor(train),
            'valid': torch.tensor(val),

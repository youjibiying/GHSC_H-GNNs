...
from data_utils import load_citation_dataset, load_LE_dataset, \
     load_yelp_dataset, load_cornell_dataset, load_NTU_ModelNet
...
class AddHypergraphSelfLoops(torch_geometric.transforms.BaseTransform):
    ...
    def __call__(self, data):
        ...
        if "edge_weight" in data:
            edge_weight = torch.cat([data.edge_weight, torch.ones_like(hyperedge_added)], -1)
            data.edge_weight = edge_weight[sorted_idx]
...
class HypergraphDataset(InMemoryDataset):
    ...
    Visual_list = ['ModelNet40_large', "NTU2012_large"]
    yelp_list = ['yelp']
    ...
    existing_dataset = cocitation_list + coauthor_list + LE_list + yelp_list + cornell_list + Visual_list
    ...
    def __init__(self, root, name,  path_to_download='./raw_data',
        feature_noise = None, transform = None, pre_transform = None, args=None):
        ...
        if "edge_weight" in self.data:
            self.data.edge_weight = self.data.edge_weight[sorted_idx]
        ...
        if "edge_weight" in self.data:
            self.data.edge_weight = self.data.edge_weight[:cidx]
    ...
    def download(self):
        for file_name in self.raw_file_names:
            ...
            elif dataset_name in self.Visual_list:

                raw_data = load_NTU_ModelNet(path=self.path_to_download, dataset = dataset_name.split("_")[0], args=self.args)

            # if dataset_name not in self.Visual_list:

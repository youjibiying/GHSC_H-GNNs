...
class H_GCNII(nn.Module):

    def __init__(self, nfeat, nlayers, nhidden, nclass, dropout, lamda, alpha, args=None):
        super(H_GCNII, self).__init__()
        self.convs = nn.ModuleList()
        for i in range(nlayers):
            self.convs.append(GCN2Conv(channels=nhidden, alpha = alpha, theta=lamda,layer=i+1,normalize=False))

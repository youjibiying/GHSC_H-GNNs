...
def load_feature_construct_H(data_dir,
    ...
    if use_mvcnn_feature_for_structure:
        mvcnn_dist = cal_distance_map(mvcnn_ft)
    if use_gvcnn_feature_for_structure:
        gvcnn_dist = cal_distance_map(gvcnn_ft)
...
def generate_H_from_dist(mvcnn_dist, gvcnn_dist, K_neigs=[10], is_probH=True,gamma=1,
    ...
    H = None
    if use_mvcnn_feature_for_structure:
        tmp = construct_H_with_KNN(mvcnn_dist, K_neigs=K_neigs,
                                    split_diff_scale=split_diff_scale,
                                    is_probH=is_probH, gamma=gamma)
        H = hyperedge_concat(H, tmp)
    if use_gvcnn_feature_for_structure:
        tmp = construct_H_with_KNN(gvcnn_dist, K_neigs=K_neigs,
                                        split_diff_scale=split_diff_scale,
                                        is_probH=is_probH, gamma=gamma)
        H = hyperedge_concat(H, tmp)
    if H is None:
